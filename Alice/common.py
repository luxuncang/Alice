...
class Common(object):
    ...
    def __init__(self, console: GraiaApp):
        ...
        self.config = config
        self.run()
        for i in TimingEvent.subclasses():
            self.loop.create_task(i.run(self.app))
    ...
    def Ariadne(self):
        ...
        Common.builtins.update(
            {"app": Common.app, "loop": Common.loop, "bcc": Common.bcc}
        )
    ...
    def Application(self):
        ...
        Common.bcc = Broadcast(loop=Common.loop)
        Common.app = GraiaMiraiApplication(
            broadcast=Common.bcc, connect_info=Session(**self.config["BotSession"])
        )
        Common.builtins.update(
            {"app": Common.app, "loop": Common.loop, "bcc": Common.bcc}
        )
...
class Monitor(metaclass=BaseMonitor):
    """监听器"""

    session: AliceSession = AliceSession  # 会话列表
    parsemap: List[ParseMapper] = Alice.parsemapper  # 匹配列表
    # ahells: List[AliceHell] = list(Alice.ahells())  # 反应列表
    ...
    async def on_message(cls, messages: AliceEventChain):
        ...
        # 会话监听器
        if await AliceSession.on_session(messages):
            return True
        ...
        try:
            for i in cls.parsemap():
                if messages.event in (GroupMessage, FriendMessage):
                    res = i.event.__ParseMapper__.match(messages.gettype(MessageChain))
                    if res:
                        logger.info(f"匹配成功: {i.event}")
        ...
        # await context(messages.event, None)
